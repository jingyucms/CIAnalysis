...
def main():
	...
	eff.GetYaxis().SetRangeUser(35,2000)
	eff.GetZaxis().SetTitle("Trigger+Reconstruction+ID efficiency")
	eff.GetZaxis().SetTitleOffset(1.3)
	eff.GetYaxis().SetTitle("simulated electron p_{T} GeV [GeV]")
	eff.GetYaxis().SetTitleOffset(2)
	eff.GetXaxis().SetTitle("simulated electron #eta")
	eff.Draw("colz")
	plotPad.SetLogy()
	...
	hCanvas.Print("effMapElectrons.pdf")
