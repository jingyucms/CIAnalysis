...
from defs import getPlot, Backgrounds, Signals, Data, path, Signals2016, zScale2016, zScale2018, Signals2016ADD
from helpers import *
...
lvals = [str(5 + i * 1) for i in range(8)]
if args.do2016:
	#lvals=["1", "10", "16", "22", "28", "34", "100k"]
	lvals = ["%.1f"%(3.5+i*0.5) for i in range(12)]; lvals.append("10")
print lvals
#helis=["LL","LR","RR"]
intfs=[""]
...
for etabin in etabins:
	...
	for antype in antypes:
		...
		if args.do2016:
			...
			addLabel="_2018"
		with open("fitFiles/addparametrization_2{0:s}_{1:s}_{2:s}_{3:s}{4:s}.json".format(antype[1],unc,etabin,csbin,addLabel),"w") as js:
			with open("fitFiles/addcounts_2{0:s}_{1:s}_{2:s}_{3:s}{4:s}.txt".format(antype[1],unc,etabin,csbin,addLabel),"w") as out:
				for intf in intfs:
					files=[]
					for point in supers[:-1]:
						# params["{0:s}{1:s}_{2:d}GeV".format(intf,heli,point)] = np.zeros(len(lvals),'float64')
						params["{0:s}_{1:d}GeV".format(intf,point)]     = [0. for j in range(len(lvals))]
						params["{0:s}_{1:d}GeV_err".format(intf,point)] = [0. for j in range(len(lvals))]
						pass
					for point in [1900+x for x in range(0,1500,200)]:
						params["{0:s}_{1:d}GeV".format(intf,point)]     = [0. for j in range(len(lvals))]
						params["{0:s}_{1:d}GeV_err".format(intf,point)] = [0. for j in range(len(lvals))]
						pass
					for i,lval in enumerate(lvals):
						...
						if i==0: print plotName, plots[plotName]
						...
						'''for ftype in ["png","C","pdf","eps"]:
							if args.do2016:
								can.SaveAs("fitPlots/addto2{1:s}_{2:s}{3:s}{4:s}_{5:s}_{6:s}_{7:s}_2016.{0:s}".format(ftype,antype[1], lval, intf, "", unc, etabin, csbin))									
							elif args.do2018:
								can.SaveAs("fitPlots/addto2{1:s}_{2:s}{3:s}{4:s}_{5:s}_{6:s}_{7:s}_2018.{0:s}".format(ftype,antype[1], lval, intf, "", unc, etabin, csbin))									
							else:	
								can.SaveAs("fitPlots/addto2{1:s}_{2:s}{3:s}{4:s}_{5:s}_{6:s}_{7:s}_2017.{0:s}".format(ftype,antype[1], lval, intf, "", unc, etabin, csbin))
						...
						for p,point in enumerate(supers[:-1]):
							...
							params["{0:s}_{1:d}GeV".format(intf,point)][i] = val
							params["{0:s}_{1:d}GeV_err".format(intf,point)][i] = err
							pass
